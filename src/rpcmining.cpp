...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "ExcelCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "ExcelCoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "ExcelCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "ExcelCoin is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
   ...
   // result.push_back(Pair("charityvalue", (int64_t)pblock->vtx[0].vout[0].nValue));  // XCL un
   // result.push_back(Pair("target", hashTarget.GetHex())); // XCL un
